...
if(!empty($_SESSION['sumsg']))
{
  unset($_SESSION['sumsg']);
				...
				/* $ckfee=mysqli_query($con,"select * from fee_detail_preivios where student='".$studrow['student_id']."' and session='".$_SESSION['session']."' and month='".$_SESSION['month1']."'");
	            $numchk=mysqli_num_rows($ckfee);
				*/
				
				$distinctmonth=mysqli_query($con,"select distinct(month) from fee_detail_preivios where student='".$studrow['student_id']."' and school='".$_SESSION['uid']."' and session='".$_SESSION['session']."'");
			...
			//*********************************************
				 ...
				 $exam=mysqli_query($con,"select * from exam_fee where month='".$_SESSION['month1']."' and session='".$_SESSION['session']."' and school='".$_SESSION['uid']."' and class='".$studrow['student_class']."'");
					...
					$examrow=mysqli_fetch_array($exam);
				  	...
				  	$combinemonth=mysqli_query($con,"select * from combinemonth where school='".$_SESSION['uid']."' and session='".$_SESSION['session']."' and month='".$_SESSION['month1']."' and class='".$studrow['student_class']."'");
					
					$rowcom=mysqli_fetch_array($combinemonth);
				
				  $search=mysqli_query($con,"select * from fee_detail_preivios where session='".$_SESSION['session']."' and school='".$_SESSION['uid']."' and student='".$studrow['student_id']."'");
		...
		//query to check previous month fee
		 ...
		 /*
			  $prevfee=mysqli_query($con,"select * from fee_detail_preivios where month='".$rowprev2['month']."' and student='".$studrow['student_id']."' and school='".$_SESSION['uid']."' and session='".$_SESSION['session']."'");
...
$prev=mysqli_query($con,"select * from month where month='".$_SESSION['month1']."'");	 
   ...
   if(isset($_POST['pay']))
   {
	 ...
	 if((!empty($_POST['latefee1']))|| (!empty($_POST['concession'])) )
	  ...
	  $receptno=mysqli_query($con,"select * from fee_detail_preivios where receiptno='".$_POST['srecipt']."' and session='".$_SESSION['session']."'");
if(mysqli_num_rows($receptno)<1)
{
$query=mysqli_query($con,"insert into fee_detail_preivios(session,class,sno,tamnt,due,fee_deposit,date,student,month,latefee,concession,school,receiptno,
current_month,remark,extra_amnt,p_year,scout,pdue,ftype,cd,cno,padv) values('".$_SESSION['session']."','".$_POST['class']."','".$_POST['sno']."','".$_POST['amt']."','$due','".$_POST['amntdeposit']."','$date','".$_POST['student']."','".$_POST['montha']."','".$_POST['latefee1']."','".$_POST['concession']."','".$_SESSION['uid']."','".$_POST['srecipt']."','$m','".$_POST['remarks']."','$extra','".$_POST['p_year']."','".$_POST['scout']."','".$_POST['pdue']."','".$_POST['ftype']."','".$_POST['cd']."','".$_POST['cno']."','".$_POST['padv']."')");
         $insertid=mysqli_insert_id();
		 
		 $update=mysqli_query($con,"update privious_fee set status='0' where sid='".$_POST['student']."'");
		...
		//$update=mysqli_query($con,"update fee_detail_preivios set admissionfee='Yes' where id='$insertid' and school='".$_SESSION['uid']."'");
		} 
...
<div class="col_4" style="min-height:400px;">
    ...
    <?php
        ...
        <?php
          ...
          <br>
          <div class="box-head">
		  <a style=" border-radius:5px; padding:5 5 5 5 ;color:#FFFFFF;font-size:16px" href="<?php echo $var."preivious"."&&divid=3"; ?>">Search Student By Class</a>
		  &nbsp;||&nbsp;
		  <a  href="<?php echo $var."p_feeledger" ?>" style="text-decoration:none; font-size:16px;  color:#FFFFFF">Ledger Previous Fee</a>
		  <a  href="<?php echo $var."p_fee" ?>" style="text-decoration:none; font-size:16px; float:right; color:#FFFFFF">Add Previous Fee</a>
		  </div>
		  ...
		  {
		  ?>
          <option value="<?php echo $rclass['class_id']; ?>" <?php if($_SESSION['student_class']==$rclass['class_id']) { ?> selected="selected" <?php } ?> ><?php echo $rclass['class']; ?></option>
            <?php
				 }
			?>
...
$rclass=mysqli_fetch_array($class);
 $search=mysqli_query($con,"select * from student where student_school='".$_SESSION['uid']."' and status='0' and rti='No' and student_class='".$rclass['class']."' order by student_name Asc");
  while($row=mysqli_fetch_array($search))
  {
  ?>
  <option value="<?php echo $row['student_id']; ?>"><?php echo $row['student_name'].'-'.$row['student_scholar']; ?></option>  
  <?php
  }
?>
			  ...
			  <tr>
			   ...
			   <tr>
			    <td>Month<br>&nbsp;<br>&nbsp;<br>&nbsp;<br>&nbsp;<br>&nbsp;<br>&nbsp;<br>&nbsp;</td>
				<td> 
				    <div style="height:150px; overflow:scroll">    
				   <input type="checkbox" name="month1[]" value="July">July<br>
                   <input type="checkbox" name="month1[]" value="August">August <br>
                   <input type="checkbox" name="month1[]" value="September">September <br>
                   <input type="checkbox" name="month1[]" value="October">October <br>
                   <input type="checkbox" name="month1[]" value="November">November <br>
                   <input type="checkbox" name="month1[]" value="December">December <br>
                   <input type="checkbox" name="month1[]" value="January">January <br>
                   <input type="checkbox" name="month1[]" value="February">February <br>
                   <input type="checkbox" name="month1[]" value="March">March <br>
                   <input type="checkbox" name="month1[]" value="April">April <br>
				    <input type="checkbox" name="month1[]" value="May">May <br>
					 <input type="checkbox" name="month1[]" value="June">June<br>
                   </td>
			</div>
			 </tr>
		...
		?> 
		 ...
		 <table style="margin:20px 0px 0px 100px; font-size:14px" width="700">
		   ...
		   <tr>
		    ...
		    <td>Receipt No</td>
			   <td>
			      <?php
				     $maxid=mysqli_query($con,"select max(id) from fee_detail_preivios where school='".$_SESSION['uid']."'");
				     $rowid=mysqli_fetch_array($maxid);
				    
				  ?>
				 <input type="text" name="srecipt" style="width:75px"  value="<?php echo $rowid['max(id)']+1; ?>" readonly/>
			   </td>
			 ...
			 ?>  
			   <?php //echo ucwords($_POST['month1']); 
			     /*
				    if(!empty($rowcom['combinemonth']))
					{ 
					     echo " & ".$rowcom['combinemonth']; 
			             $ex=explode(",",$rowcom['combinemonth']); $count=count($ex); $count=$count+1;   
			        }
					*/ 
		   ...
		   <tr>
		   <td>School Rno</td>
		   <td> <input type="text" name="sno" style="width:100px" /></td>
		   <td>Admission No</td>
         ...
         <div id="txtHint1">
				  ...
				  <?php		
					...
					<?php
				    $caut=mysqli_query($con,"select * from privious_fee where session='".$_SESSION['session']."' and sid='".$studrow['student_id']."' and status='1'");
				    $rowacaut=mysqli_fetch_array($caut);   
					?>
					<tr>
					<td></td>	
					<td style="color:rgb(53, 0, 255)">Privious Year Fee</td>
					<td><?php echo $rowacaut['amt']; ?></td></tr>
				    <input type="hidden" name="p_year" value="<?php echo $rowacaut['amt']; ?>">
								...
								<tr>
									...
									<td><b><?php   $search1=mysqli_query($con,"select * from fee_detail_preivios where session='".$_SESSION['session']."' and school='".$_SESSION['uid']."' and student='".$studrow['student_id']."' order by id desc limit 1"); 
				              ...
				              <tr>
									...
									<td><div><?php echo $rowacaut['amt']+$duerow['due']+$fine1; ?></div>
									 <input type="hidden" name="scout" value="<?php echo $rowacaut['amt']+$duerow['due']+$fine1; ?>">
							          <input type="hidden" name="cover" value="<?php echo $val+$lab1['fee']+$schrow['fee']+$compt['fee']; ?>">
							 </td>	
                            ...
                            <tr>
							    <td><input type="hidden" name="computerfee" value="<?php echo $tcomp; ?>"><input type="hidden" name="examfee" value="<?php echo $texam; ?>">
							   </td>
							    <td>
								<input type="hidden"  name="report" style="width:80px;" value="<?php echo $tdev; ?>"/>
								<input type="hidden" name="dairy" value="<?php echo $tspo; ?>"></td>
								<td>
								<input type="hidden" name="montly_fee" value="<?php echo $tmm; ?>" style="width:80px;">
								<input type="hidden" name="misc" value="<?php echo $tlab; ?>" style="width:80px;">
							...
							<tr>
									...
									<td><div id="tamt"><?php echo $rowacaut['amt']+$duerow['due']+$fine1-$duerow['extra_amnt']; ?></div>
									
									<input type="hidden" name="amt" value="<?php echo $rowacaut['amt']+$duerow['due']+$_SESSION['fine']-$duerow['extra_amnt']; ?>">
									</td>
								...
								<td></td>
							    <td><b>Fee Type</</td>
							    <td> <label><input type="radio" name="ftype" value="Cash" checked="checked">Cash</label>
                                <label><input type="radio" name="ftype" value="Cheque">Cheque</label></td>
								</tr>	
								  <tr>
							   <td></td>
							   <td></td>
                               ...
                               <div class="green" style="display:none"></div>
                              <td></td>
...
?>
<td>
<a href="javascript:void(0)" style="color:#FF0000" onClick="return popitup('https://smarterponline.com/kmps/school/p_getreceipt.php?id=<?php echo $_SESSION['studentid']; ?>')">
