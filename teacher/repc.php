...
<div class="enquiry">
<h2 style="text-transform:uppercase; color:#006633; font-size:14px;margin-top:20px;">Complaint Details</h2>
<a href="./?pageid=reppp" style="color:#FFFFFF;float:right; background-color:#FF0033; margin-top:-15px; padding:6px; font-size:15px">Add New Complaint</a>
</div>
<?php
if(!empty($_GET['did']))
{
$query=mysqli_query($con,"delete from comp_box where id='".$_GET['did']."'");	 
 
}
$maxid=mysqli_query($con,"select count(id) from comp_box where tid='".$_SESSION["userid"]."' and session='".$_SESSION['session']."'");
$maxrow=mysqli_fetch_array($maxid);
$rowmax=mysqli_fetch_array($maxid);
?>
<div class="col_4">
<div class="form-style-2-heading">Total Complaint - <?php echo $maxrow['count(id)']; ?></div>
<table class="table table-bordered" id="sample_1" style="font-size:11px; width:100%; ">
              ...
              <tbody>
	<?php
    $sql=mysqli_query($con,"select * from comp_box where session='".$_SESSION['session']."' and tid='".$_SESSION['userid']."'");
	...
	while($row=mysqli_fetch_array($sql))
	{
		?>
               <tr>
                  ...
                  <td><a href="<?php echo $var."compp&id=".$row['id']; ?>" target="_blank">Edit</a>|
				  
				   <a href="<?php echo $var."repc&did=".$row['id']; ?>" onClick="return confirmation();">Delete</a>
				  </td>
              </tr>
