...
<div class="full_div">
<br clear="all" />
<div class="left_sect"><img src="images/FEE Management/feehome.png" /><a href="./?pageid=fee_leadger">
...
<form action="#" method="post" enctype="multipart/form-data">
				 <div class="box-head" style="font-size:18px">
					 <a style=" border-radius:5px; padding:5 5 5 5 ; color:#FFFFFF; font-size:16px" href="<?php echo $var."hostel_session"."&&divid=1"; ?>"><a style=" border-radius:5px; padding:5 5 5 5 ;color:#FFFFFF;font-size:16px" href="<?php echo $var."hostel_session"."&&divid=2"; ?>"> Total Fee Collection By Class In Session</a>
						</div>
        ...
        <?php
		 ...
		 <div id="printablediv" style="width: 100%;">
          ...
          <h2 align="center" style="margin-top:9px; color:#990033;font-weight:bold;">Hostel Fee, Session: <?php echo $_POST['session']; ?></h2>
		  <?php if($_POST['class'])
		  {
		  ?>
		  ...
		  ?>
		   ...
		   <td>
		   <?php
		   $selrc=mysqli_query($con,"select * from definefee_hostel  where class='".$row['class']."' and session='".$_POST['session']."'");	
$rowselrec=mysqli_fetch_array($selrc);
		  ...
		  // $new_std = $rownum1['count(student_id)']*7000; 
		   ...
		   <td>
		   <?php
		   $search=mysqli_query($con,"select sum(fee_deposit),sum(latefee),sum(concession) from fee_detail_hostel where session='".$_POST['session']."' and school='".$_SESSION['uid']."' 
		   and class='".$row['class']."'");
		   ...
		   <?php /*?><tr>
			...
			$numclass1=mysqli_query($con,"select * from student where student_class='".$_POST['class']."' and student_session='".$_POST['session']."' and hostel_status='Active'");
			...
			if($rownum1['std_type']=='New')
			{
			$selrc=mysqli_query($con,"select * from definefee_hostel  where class='".$_POST['class']."' and session='".$_POST['session']."'");	
            $rowselrec=mysqli_fetch_array($selrc);
 			$amnt = $rowselrec['amnt']+5000; 
			$total1=$val2+$amnt;
	        echo $total1;
	        $total2+=$total1;
			}
