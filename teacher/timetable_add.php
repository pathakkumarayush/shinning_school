...
if(isset($_POST["save"]))
{
	$result=mysqli_query($con,"select * from exam where examination='".$_POST["ename"]."' and session='".$_POST["session"]."' and class='".$_POST['class']."' and edate='".$_POST['date']."' and school='".$_SESSION["schoolname"]."' ")or die(mysqli_error());
	...
	mysqli_query($con,"insert into exam(examination,session,sdate,school,marks,subject,class,edate)values('".$_POST["ename"]."','".$_POST["session"]."','".$_POST['date']."','".$_SESSION['schoolname']."','".$_POST['marks']."','".$_POST['sub']."','".$_SESSION['q']."','".$_POST['edate']."')");
   $msg="inserted Successfully";
 }
}
?>
<?php
$exli=mysqli_query($con,"select * from examination where examination_name='".$_GET['examinationname']."' ");
...
<table cellspacing="5" style="margin-left:105px;">
 ...
 <tr>
  ...
  <tr>
    <td>Date :</td>
    <td><input type="text" name="edate" value="<?php echo $exli1["examination_date"]; ?>" readonly="readonly" class="tb5"  /></td>
  </tr>
</table>

<div style="border:#CCC solid 1px; width:460px; margin-left:100px; position:relative;">
...
<select name="class"  onchange="getsubject(this.value)" class="select">
<option>Select Class</option>
<?php $cla=mysqli_query($con,"select *  from class where school='".$_SESSION['uid']."' ");
while ($cla1=mysqli_fetch_array($cla)) { ?>
<option value="<?php echo $cla1["class"].$cla1["class_section"]; ?>" <?php if($cla1["class"].$cla1["class_section"]==$_SESSION['q']){ ?> selected="selected" <?php } ?>><?php echo $cla1["class"].$cla1["class_section"]; ?></option>
<?php } ?>
</select></td>
</tr>
...
<td><div id="txtHint"><b>Subject will be listed here.</b></div></td>
	...
	<?php
	$sql="SELECT * FROM subjects WHERE class = '".$_SESSION['q']."' and school='".$_SESSION["uid"]."' and session='".$_SESSION['session']."'";
	...
	$sub = $row['name'];
	 ...
	 ?>
	   ...
	   <td>
	 <input name="date" type="text" id="demo<?php echo $i; ?>" style=" width:100px;" class="tb5"><a href="javascript:NewCal('demo<?php echo $i; ?>','ddmmmyyyy')"><img src="css/images/cal.gif" width="20" height="15" border="0" alt="Pick a date<?php echo $i; ?>" style=" padding-top:10px;"></a></td>
	<td>
	<input type="text" name="marks"   style="width:60px; margin-right:2px; float:right" class="tb5"></td>
...
<br /><br /><br />
<div class="box-head">
<h2><?php echo ucwords($_SESSION["exam_name"])." Timetable For ".$_SESSION['q'];  ?></h2>
</div>
...
<?php
	$i=1;
	$exam=mysqli_query($con,"select * from exam where examination='".$_SESSION["exam_name"]."' and session='".$_SESSION['examinationsession']."'  and class='".$_SESSION['q']."' and  school='".$_SESSION["schoolname"]."' ");
