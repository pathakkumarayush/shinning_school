...
{
$sel=mysqli_query($con,"select * from cautionfee where session='".$_SESSION['session']."' and class='$class'");
if(mysqli_num_rows($sel)<1)
{	
$queryw=mysqli_query($con,"insert into cautionfee(class,fee,session) values('$class','".$_POST['fee']."','".$_SESSION['session']."')") or die(mysqli_error());
...
{
 $err="Caution Money Fee Already Exist";
}
...
$selrc=mysqli_query($con,"select * from cautionfee where id='".$_GET['id']."'");	
$rowselrec=mysqli_fetch_array($selrc);	
	}
if(isset($_POST['submit1']))
{
$queryupdate=mysqli_query($con,"update cautionfee set fee='".$_POST['fee']."' where id='".$_POST['id']."'") or die(mysqli_error());	

?>
<script type="text/javascript">
	  window.location = "<?php echo $var."addcautionmoney&&uid=update Sucessfully"; ?>";
	</script>
 <?php
}
?>
<?php
 if(!empty($_GET['did']))
 {
	 ...
	 $del2=mysqli_query($con,"delete from cautionfee where id='".$_GET['did']."'"); 
     
 ?>
  <script type="text/ecmascript">
	  window.location = "<?php echo $var."addcautionmoney&&dmsg=Deleted Sucessfully"; ?>";
	</script>
...
<div id="container">
			...
			<div id="content" style="border:#F00 0px solid; width:990px; height:auto">
				...
				<div  style="border:#FF0000 0px solid; height:400px; margin-top:20px">
				   <img src="css/images/1365164012_data_management.png" style="margin-left:20px;height:80px; width:80px"><br />
                    	<span style=" color:#000000; font-size:18px; margin-top:-20px; margin-left:10px">Caution Fee</span>

                        <div style="border:#900 2px solid; margin-top:10px"></div>
                <a href="./?pageid=fee_str">Fee Structure</a> >>Add Cautionfee</a>
    ...
    <?php
     ...
     <table border="0" style="margin:10px 0px 0px 20px">
	    ...
	    while($meta = mysqli_fetch_array($memo1))
	   {
		 ...
		 $selrc=mysqli_query($con,"select * from cautionfee where class='$class'");	    
   ...
   <tr>  
       ...
       <td><?php if($num>0) {  ?> <a href="<?php echo $var."addcautionmoney&&id=".$selstruc['id'];?>" style="color:#FF0000">edit</a>/<a href="<?php echo $var."addcautionmoney&&did=".$selstruc['id'];?>" onClick="return confirmation();" style="color:#FF0000">Delete</a> <?php } ?></td>  
    </tr>
    <?php
    $i++;
	}
