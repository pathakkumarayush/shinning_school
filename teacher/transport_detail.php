...
$chroute=mysqli_query($con,"select * from rout_allocation where rout_no='".$_POST['rout_no']."' and school='".$_SESSION['uid']."' and session='".$_SESSION['session']."' ");
if(mysqli_num_rows($chroute)<1)
{
$stopage=implode(",",$_POST['stopage']);
$vehcle=implode(",",$_POST['vehcle']);
  $query=mysqli_query($con,"insert into rout_allocation(rout_no,stopage,vehcle,school,session) values('".$_POST['rout_no']."','$stopage','$vehcle','".$_SESSION['uid']."','".$_SESSION['session']."')");
?>
<script type="text/javascript">
window.location="<?php echo $var."transport_detail&msg=Inserted Successfully";   ?>";
</script>
<?php
}
else
  {
    $err="Rout already engaged";
  }
}
if(isset($_GET['id']))
{
$groute=mysqli_query($con,"select * from rout_allocation where id='".$_GET['id']."' and school='".$_SESSION['uid']."'");
...
$stopage=implode(",",$_POST['stopage']);
 $vehcle=implode(",",$_POST['vehcle']);
 $upd_rout=mysqli_query($con,"update rout_allocation set rout_no='".$_POST['rout_no']."',stopage='$stopage',vehcle='$vehcle' where id='".$_GET['id']."'");
?>
<script type="text/javascript">
window.location="<?php echo $var."transport_detail&msg=Update Successfully";   ?>";
</script>
<?php
}
...
if(!empty($_GET['did']))
{
$del_rout=mysqli_query($con,"delete from rout_allocation  where id='".$_GET['did']."' and school='".$_SESSION['uid']."'");
?>
<script type="text/javascript">
window.location="<?php echo $var."transport_detail&msg=Deleted Successfully";   ?>";
</script>
<?php
}
...
<div class="col_4" style="margin-top:0px;" >
    ...
    <?php
	 ...
	 ?>
         ...
         <table border="0" style="margin:10px 0px 0px 20px">
			...
			<option value="Rout 1A">Rout 1A</option>
			<option value="Rout 1B">Rout 1B</option>
		    <option value="Rout 2A">Rout 2A</option>
			<option value="Rout 2B">Rout 2B</option>
			<option value="Rout 3A">Rout 3A</option>
			<option value="Rout 3B">Rout 3B</option>
		    <option value="Rout 4A">Rout 4A</option>
			<option value="Rout 4B">Rout 4B</option>
			<option value="Rout 5A">Rout 5A</option>
			<option value="Rout 5B">Rout 5B</option>
		    <option value="Rout 6A">Rout 6A</option>
			<option value="Rout 6B">Rout 6B</option>
			<option value="Rout 7A">Rout 7A</option>
			<option value="Rout 7B">Rout 7B</option>
		    <option value="Rout 8A">Rout 8A</option>
			<option value="Rout 8B">Rout 8B</option>
			<option value="Rout 9A">Rout 9A</option>
			<option value="Rout 9B">Rout 9B</option>
			<option value="Rout 10A">Rout 10A</option>
			<option value="Rout 10B">Rout 10B</option>
			 </select>
	...
	?>
         ...
         <div class="table" style="border:#FF0000 0px solid; min-height:200px;overflow:scroll">
      ...
      <tr>
        <td><?php echo $i;  ?></td>
       <td><?php echo ucwords($row['rout_no']);  ?></td> 
	   <td><?php 
	   echo $row['stopage'];  
	   ?></td>
	    <td><?php echo $row['vehcle'];  ?></td>
	   <td><a style="color:#CC0033" href="<?php echo $var."transport_detail"."&&id=".$row['id']; ?>">Edit</a>/<a style="color:#CC0033" href="<?php echo $var."transport_detail"."&&did=".$row['id']; ?>" onClick="return confirmation();">Delete</a></td>
	 </tr>
	<?php
	  $i++;
	 }
