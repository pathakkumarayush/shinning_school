...
<center><h2 style="text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">New Admission</h2></center>
</div>
<?php
$maxid=mysqli_query($con,"select count(student_id) from student where std_type='new' and student_school='".$_SESSION["uid"]."' and student_session='".$_SESSION['session']."'and status='0'");
...
<table class="table table-bordered" id="sample_1" style="font-size:12px; ">
              ...
              <tbody>
			  <?php
    $sql=mysqli_query($con,"select * from student where std_type='New' and student_school='".$_SESSION['uid']."' and  student_session='".$_SESSION['session']."' and status='0' order by student_name Asc");
	...
	while($row=mysqli_fetch_array( $sql))
	{
		?>
               <tr>
				  ...
				  <td><a href="<?php echo $var."admission&upstudid=".$row['student_id']; ?>">View</a>||<a href="<?php echo $var."new_student&did=".$row['student_id']; ?>" onClick="return confirmation();">Delete</a></td>
              </tr>
