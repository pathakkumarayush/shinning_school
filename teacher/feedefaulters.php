...
<?php
  if(isset($_POST["send"]))
  {
   ...
   $sub="Fee Due Message";
   $nmsg="Fee For the month ".$_POST['amnt']." amount ".$d." has been due please pay the amount as soon as possible.";	
...
<img src="images/buttonGoBack.png"  style="float:right; width:150px; height:60px;"/></a></div>
	<div class="shell"  >
		<div id="main" >
		   ...
		   <tr><td></td><td><a style="border-radius:5px;padding:5px;color:#FFFFFF; background-color:#CC66CC;font-size:15px;border-top-left-radius:2px;border-top-right-radius:2px; text-decoration:none;" href="">All fee Defaulter Student</a></td></tr>
		<tr><td>&nbsp;</td></tr>
		   <tr>
           <td>Select Instalment</td><td><select name="month1"  class="select" style="width:170px">
                   <option value="Select Month">Select Instalment</option>
		            <option value="April">Instalment-I</option>
                   <option value="August">Instalment-II</option>
                   <option value="December">Instalment-III</option>
     ...
     <div class="table" style="border:#33cc66 20px solid; height:2020px; margin:0px 0px 0px 0px; overflow:scroll">
     <a href="javascript:void(0)" style="color:#FF0000" onClick="return popitup('https://smarterponline.com/shining/school/defaulters.php?month=<?php echo $_POST['month1']."&ses=".$_SESSION['session'];;  ?>')">     <input type="button" value="Print List " style="width:200px;float:left; position:absolute; margin-top:-20px;" ></a>
			...
			<h2 align="center" style="margin-top:10px; color:#006633;text-transform:uppercase; font-weight:bold;">FEE DEFAULTER ALL CLASS  - Session: <?php echo $_SESSION['session'];?></h2>
...
$search=mysqli_query($con,"select * from student where student_session='".$_SESSION['session']."' and status='0' order by student_class asc");
		...
		$instd=mysqli_query($con,"select * from instdetail where session='".$_SESSION['session']."' and month='".$_POST['month1']."' and class='".$studrow['student_class']."'");
		$rowinst=mysqli_fetch_array($instd);
		
		$exam=mysqli_query($con,"select * from exam_fee where session='".$_SESSION['session']."' and month='".$_POST['month1']."' and class='".$studrow['student_class']."'");
    ...
    <td><input type="checkbox" name='formDoor[]' value="<?php echo $studrow['student_id']; ?>"  id='chk<?php echo $i; ?>' /></td>
	 ...
	 <td><?php echo $rowinst['inst_type']; ?>
	 <input type="hidden" name="amnt" value="<?php echo $_POST['month1']; ?>" />
	 </td>
	 <td><?php echo $rowinst['amnt']+$rowex['fee'];?></td>
	 <td><?php echo ucwords($studrow['student_session']);?></td>
	 
     <input type="hidden" name="due[<?php echo $studrow['student_id']; ?>]" value="<?php echo $rowinst['amnt']+$rowex['fee']; ?>"  style="width:80px"  />
    </tr>
    <?php
    $i++;
	$num4="";
