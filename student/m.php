...
<div class="hero-unit-table">   
  ...
  <?php
$searc=mysqli_query($con,"select * from student where uid='".$_SESSION['userid']."' and student_session='".$_SESSION['session']."' ");
$studro=mysqli_fetch_array($searc);
echo $studro['student_name'];
$class = $studro['student_class'];
$id = $studro['uid'];
$term1 = "TERM2";
$uid=$studro['uid'];
?>
...
$err=mysqli_query($con,"select sub_term from terms where term='$id[0]'");
$len1=mysqli_num_rows($err);
$t=0;
while($t<=$len1)
{
$final_cal[$t]=0;
...
while($row=mysqli_fetch_row($err))
{
	$te_cal=0;
?>
<?php 
