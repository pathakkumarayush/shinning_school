...
<?php
$res_msg1=mysqli_query($con,"select count(id) from sendmsg where reciever='$std_id'  and is_read='0'")or die(mysqli_error());
...
<a  style="background-color:#EF7F1A; padding-left:7px; padding-right:7px; padding-bottom:2px; padding-top:2px; border-radius:4px; border:#FFFFFF solid; color:#FFFFFF;margin-left:10px;" href="<?php echo $var."inbox"; ?>">Message In-Box <?php if($row_msg11['count(id)']>0) { ?> 
<span style="color:#fff">&nbsp; <?php  echo  "(".$row_msg11['count(id)'].")"; ?> </span> <?php }  ?>
</a>
...
<table class="table table-bordered" id="sample_1" style=" color:#000000; margin-left:5px;" width="98.7%">
			 ...
			 <tbody>
			<?php
$res_msg=mysqli_query($con,"select * from sendmsg where reciever='$std_id' ORDER BY id DESC")or die(mysqli_error());
