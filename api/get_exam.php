...
$query = "SELECT * FROM examination WHERE examination_session = '$session_esc'";
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    $response['message'] = count($exams) . ' exam(s) found.';
    $response['data'] = $exams;
} else {
    $response['status'] = false;
    $response['message'] = 'No exams found for the given session.';
}
