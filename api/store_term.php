...
$examination_id = isset($input['term_id']) ? trim($input['term_id']) : '';
...
if ($exam_id_esc > 0) {
    ...
    if (mysqli_query($con, $updateQuery)) {
        if (mysqli_affected_rows($con) > 0) {
            ...
            $response['message'] = "Term updated successfully (by ID).";
        } else {
            $response['message'] = "No term found with given ID.";
        }
    } else {
    ...
    $checkQuery = "SELECT * FROM examinationa 
    ...
    if ($checkResult && mysqli_num_rows($checkResult) > 0) {
        ...
        // Insert new
        $insertQuery = "INSERT INTO examinationa (examination_name, examination_date, examination_session, school) 
                        VALUES ('$name_esc', '$date_esc', '$session_esc', 'scottish')";
        if (mysqli_query($con, $insertQuery)) {
            $response['status'] = true;
            $response['message'] = "Term added successfully.";
            $response['data'] = ['id' => mysqli_insert_id($con)];
        } else {
            $response['message'] = "Failed to add term: " . mysqli_error($con);
        }
    }
}
