...
<form method="post" action="">
	  <table width="896" border="0">
        <tr class="item" >
          ...
          <td width="209">Book number </td>
          <td width="550"><input class="tb5" type="text" name="bnm" /></td>
        </tr>
        <tr>
          <td height="43">&nbsp;</td>
          <td>&nbsp;</td>
...
<?php
	$bnm=$_POST["bnm"];
	require_once("../db.php");
	$qry="select * from issuebook where bookno='$bnm'  ";
	$result=mysqli_query($con,$qry);
	$row=mysqli_fetch_array($result);

	 $qry1="select * from addbook  where bookno='$bnm' ";
	  $result=mysqli_query($con,$qry1);
	  $row1=mysqli_fetch_array($result);
	  mysqli_close();
	  if($row1["status"]==0)
	  {
	  echo "<div align='center' style='font-family:Arial, Helvetica, sans-serif; font-size:18px; color:#FF0000; font-weight:bold;' > <font color='red' > Sorry book has not been issued  </font> </div>";
	  }
	else
	{
...
$query=mysqli_query($con,"insert into renewbook(bno,name,sname,sid,idate,ddate,date,amt,type) values('".$_POST['bnm']."','".$_POST['title']."','".$_POST['nm']."','".$_POST['bid']."','".$_POST['doi']."','".$_POST['dd']."','$d','".$_POST['fam']."','student') "); 
