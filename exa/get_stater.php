...
require_once("../db.php");
$_POST["student_id"];
$exa=mysqli_query($con,"select * from fee_detail where student='".$_POST["student_id"]."'");
...
<td style="color:#CCCC33">Total Fee</td>
<td>
<?php
...
$total=mysqli_query($con,"select * from definefee  where class='$cls'");
$tamt=mysqli_fetch_array($total);


echo $tt = $tamt['amnt']+$tamt['actfee']+$ad+$tfee+500;
?> </td>
<td style="color:#CCCC33">&nbsp;&nbsp;Total Fee Pay</td>
<td>
<?php 
if($val4=='')
{
...
<tr><td style="color:#CCCC33">Concession </td><td><?php echo $tcon; ?></td><td style="color:#CCCC33">Fine (Late Fee) </td><td><?php echo $tfine; ?></td></tr>
				  ...
				  <div style="border:#CCCCCC 2px solid; margin:20px 0px 0px 50px; height:260px; width:550px;overflow:scroll;">
                  <?php
		          $shkstd=mysqli_query($con,"select * from fee_detail where student='".$_POST["student_id"]."'");
