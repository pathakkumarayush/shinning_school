...
<?php
   ...
   {
   $d=date("Y-m-d");
   $insert=mysqli_query($con,"insert into sendmsg (sender,sender_user,reciever,sub,msg,status,date,session,type,class)
 values('teacher','".$_SESSION['userid']."','".$_POST['reciver']."','".$_POST['sub']."','".$_POST['msg']."','".Yes."','$d','".$_SESSION['session']."','".$_POST['type']."','".$_POST['class']."')");
	 ...
	 $sedurl="http://www.businesssms.co.in/sms.aspx?Id=eliteschool2100@gmail.com&Pwd=adminelite&PhNo=".$PhNo."&text=".$nmsg;
...
<?php
$res_msg=mysqli_query($con,"select * from sendmsg where reciever='".$_SESSION['userid']."' and id='".$_GET['id']."'")or die(mysqli_error());
$row_msg=mysqli_fetch_array($res_msg);

$updmsg=mysqli_query($con,"update sendmsg set is_read='1' where reciever='".$_SESSION['userid']."' and id='".$_GET['id']."' ");
?>
<div id="container">
<div class="shell">
...
<a  style="background-color:#006600; padding-left:7px; padding-right:7px; padding-bottom:2px; padding-top:2px; border-radius:4px; border:#FFFFFF solid; color:#FFFFFF;margin-left:10px;" href="<?php echo $var."inbox"; ?>">Inbox <?php if($row_msg11['count(id)']>0) { ?> 
<span style="color:#FF0000">&nbsp; <?php  echo  "(".$row_msg11['count(id)'].")"; ?> </span> <?php }  ?>
</a>

<a href="./?pageid=sentbox" style="background-color:#006600; padding-left:7px; padding-right:7px; padding-bottom:2px; padding-top:2px; border-radius:4px; border:#FFFFFF solid; color:#FFFFFF;margin-left:10px;">Sent Box</a>

<a href="./?pageid=compose" style="background-color:#006600; padding-left:7px; padding-right:7px; padding-bottom:2px; padding-top:2px; border-radius:4px; border:#FFFFFF solid; color:#FFFFFF;margin-left:10px;">Compose</a>
<br clear="all" />
<br clear="all" />
</div>		
<?php
   ...
   {
   ?>
  <label style="color:#e91e63; font-weight:bold"><?php  echo $msg; ?></label>
   <?php
   }
...
<tr>
 <td><a href="<?php echo $var."viewmessage&id=".$_GET['id']."&id2=".$rowteach['student_id'];  ?>" style="color:#0033CC">
...
<?php
   ...
   <table cellpadding="table3" style="">
      <tr> 
       <td><b>To-<?php echo $rowteach['student_name']; ?></b>
	   <input type="hidden" name="reciver" value="<?php echo $rowteach['student_id'];  ?>">
	   <input type="hidden" name="class" value="<?php echo $rowteach['student_class'];  ?>">
	   <input type="hidden" name="contact" value="<?php echo $rowteach['student_contactno'];  ?>">
	   </td>
       </tr>
	  <tr>
	    <td>&nbsp;</td>
