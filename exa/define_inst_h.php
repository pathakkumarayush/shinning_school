...
$del=mysqli_query($con,"delete from instdetail_hostel where class='".$_GET['did']."' and session='".$_SESSION['session']."'"); 
}
...
$b= count($_POST['month']);
 ...
 $classq=mysqli_query($con,"select * from instdetail_hostel where  class='".$_POST['class']."' and session='".$_SESSION['session']."'");
 ...
 $query=mysqli_query($con,"insert into instdetail_hostel (class,inst_type,amnt,session) values('".$_POST['class']."','$insttype','$v','".$_SESSION['session']."')");
}
  foreach($_POST['month'] as $m=>$v)
  {
    $insttype= "Instalment".$m;
  $upd=mysqli_query($con,"update instdetail_hostel set month='$v' where class='".$_POST['class']."' and inst_type='$insttype'");
  }
...
$selrc=mysqli_query($con,"select * from definefee_hostel where  session='".$_SESSION['session']."' and class='".$_POST['class1']."'");
...
$i=1;
$selrc1=mysqli_query($con,"select * from definefee_hostel where  session='".$_SESSION['session']."' and class='".$_POST['class1']."'");
...
<div class="table" style="border:#FF0000 0px solid; height:520px; overflow:scroll">
       ...
       <?php
        $memo=mysqli_query($con,"select distinct(class) from instdetail_hostel where session='".$_SESSION['session']."' ");
		
		$i=1;
	while($rowmemo=mysqli_fetch_array($memo))
	{
	  $memo1=mysqli_query($con,"select * from instdetail_hostel where session='".$_SESSION['session']."' and class='".$rowmemo['class']."' ");
