...
<?php
 if(!empty($_GET['id']))
{
$getdetail=mysqli_query($con,"select * from fee_detail where id='".$_GET['id']."' and school='".$_SESSION['uid']."' and session='".$_SESSION['session']."' order by id desc limit 1");
$rowfeedetail=mysqli_fetch_array($getdetail);
$reg=mysqli_query($con,"select * from student where student_id='".$rowfeedetail['student']."' and student_school='".$_SESSION['uid']."' and student_session='".$_SESSION['session']."'");
	...
	<div class="shell"  >
			...
			<div id="content" style="border:#F00 0px solid; width:990px; height:auto">
				...
				<div  style="border:#FF0000 0px solid; height:400px; margin-top:20px">
				   <img src="css/images/1365164854_elementary_school.png" style="margin-left:20px;height:80px; width:80px"><br />
                    	<span style=" color:#000000; font-size:18px; margin-top:-20px; margin-left:10px">Ledger Detail</span>
				 ...
				 {
				 ?>     
			      <a href="./?pageid=studentledger">Student Ledger</a> >>Student Ledger Detail</a>
                 <?php
				 }
				 else
				 {
				 ?>
	             <a href="./?pageid=studentledger2&ushiningtd=<?php echo $_SESSION['userid'];  ?>">My Ledger</a> >>My Ledger Detail</a>
				 <?php
				   }
				 ?>
	   ...
	   ?>
		 ...
		 ?>
		   ...
		   <?php
		    $getdetail=mysqli_query($con,"select * from fee_detail where id='".$_GET['id']."' and school='".$_SESSION['uid']."' and session='".$_SESSION['session']."' ");
$rowfeedetail=mysqli_fetch_array($getdetail);
		      $m = $rowfeedetail['instalment'];
		   
		     $selrc=mysqli_query($con,"select * from instdetail where class='".$rowfeedetail['class']."' and session='".$_SESSION['session']."' and inst_type='$m' ");
			 $rowinstalment=mysqli_fetch_array($selrc);
				$i=0;
				?>
				<tr>
				   <td><?php echo $i+1;   ?></td>
				   <td><?php echo $m;   ?></td>
				   <td><?php echo $rowinstalment['amnt'];  
				        $i=$i+1;
				    ?></td>
				</tr>
				
			<?php
			    $extahead=mysqli_query($con,"select * from  fee_memo where session='".$_SESSION['session']."' and feetype='".$rowfeedetail['month']."' ");
			 ...
			 <?php
								 ...
								 <tr> 
								    <td><?php echo $i+1;   ?></td>
									      ...
									      <?php
										       $extahead=mysqli_query($con,"select * from  fee_structure where session='".$_SESSION['session']."' and class='".$rowfeedetail['class']."' ");
	...
	$examrow1=mysqli_fetch_array($exam1);
			  ...
			  $rowtr=mysqli_fetch_array($querytr);
							...
							<tr>
								...
								<td><b>
								<?php  $search1=mysqli_query($con,"select * from fee_detail where session='".$_SESSION['session']."' and school='".$_SESSION['uid']."' and student='".$rowstud['student_id']."' and id<'".$_GET['id']."' order by id desc limit 1");
