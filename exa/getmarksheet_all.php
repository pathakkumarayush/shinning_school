...
.sn1{ width:125px!important;}
	 ...
	 $search=mysqli_query($con,"select * from student where student_class='".$_GET['class']."' and student_session='".$_SESSION['session']."' and status='0' order by student_name Asc");
	  ...
	  ?>	
      <div style="width:1050px;height:1531px; border:6px #CC0000 solid;">
...
<img src="upload/<?php echo $rowstud["student_id"].$_SESSION['uid'].$rowstud['student_img'];  ?>" style="height:130px; margin-left:10px; width:115px; margin-top:12px; " />
</div>
...
<tr style="line-height:40px;"><td style="width:150px;border-right:3px #303192 solid; color:#CC0000">&nbsp;&nbsp;Total Marks
<?php
$searchm=mysqli_query($con,"select sum(obtainmarks) from marks where student='".$rowstud['uid']."' and ses='".$_SESSION['session']."' and exam='".$_GET['exam']."'");
$rowmk=mysqli_fetch_array($searchm);
$amtrc = $rowmk['sum(obtainmarks)'];

		   
?>

</td>
<td style="border-right:3px #303192 solid;width:90px;"><center><?php echo  $amtrc; ?></center></td>
<td style="border-right:3px #303192 solid;width:150px;color:#CC0000"><center>&nbsp;&nbsp;Percentage</center></td>
<td style="border-right:3px #303192 solid;width:90px;"><center><?php $perg = $amtrc/$mid; echo substr($perg, 0, 4); ?>%</center></td>
