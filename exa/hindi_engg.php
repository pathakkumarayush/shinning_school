...
<div class="full_div">
	 ...
	 $searchs=mysqli_query($con,"select * from hindi_english_n where student='".$studrow['student_id']."' and class='".$studrow['student_class']."' and subject='".$_POST['subject']."' and exam='".$_POST['exam']."' and session='".$_SESSION['session']."'");
		...
		<td>	
		 ...
		 <td >
      <?php if(!empty($studrows['vaca']) || !empty($studrows['con']) || !empty($studrows['reada']) || !empty($studrows['lettersa']) || !empty($studrows['formationa']) ){ ?>
...
var data_str= "cls="+cls+"&idm="+idm+"&exam="+exam+"&subject="+subject+"&vaca="+vaca+"&con="+con+"&reada="+reada+"&lettersa="+lettersa+"&formationa="+formationa;
