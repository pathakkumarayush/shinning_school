...
<div class="full_div">
		...
		<!-- <div style="width:1322px; height:50px; background-color:#FF0000; margin-top:10px;">
		 ...
		 {
		  $searchs=mysqli_query($con,"select * from co_scholastic where student='".$studrow['student_id']."' and class='".$studrow['student_class']."' and exam='".$_POST['exam']."' and session='".$_SESSION['session']."'");
		 ...
		 <td >
            <input type="text" name="music" value="<?php echo $studrows['music']; ?>" id="music<?php echo $i;?>" style="width:50px;">
         </td>
		 <td >
		 ...
		 <td >
		  ...
		  <td >
            <input type="text" name="discipline" value="<?php echo $studrows['discipline']; ?>" id="discipline<?php echo $i;?>" style="width:45px;">
         </td>
		 
		 <td >
      <?php if(!empty($studrows['music']) || !empty($studrows['arts']) || !empty($studrows['sport']) || !empty($studrows['discipline']) ){ ?>
...
var discipline= document.getElementById('discipline'+hid).value;
...
var data_str= "cls="+cls+"&idm="+idm+"&exam="+exam+"&music="+music+"&arts="+arts+"&sport="+sport+"&discipline="+discipline;
 
$.ajax({
type:"POST",
url:"add_personalityy.php",
data:data_str,
success:function(){
