...
<h2 style="float:left; margin-left:10px; text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">Inactive Staff Details </h2>
...
<a href="./?pageid=group_d" style="color:#FFFFFF;float:right; margin-left:-20px; background-color: #000; margin-top:10px; padding:6px; font-size:18px">Inactive Staff</a>
</div>
<?php
if(!empty($_GET['did']))
{
...
?>
<?php
$maxid=mysqli_query($con,"select count(teacher_id) from teacher where status='Inactive' and teacher_session='".$_SESSION['session']."'");
...
$myArr=array("S.No.","Teacher Name","husband/Father","Mobile","Qualification","address");
$excel->writeLine($myArr);


$qry=mysqli_query($con,"select * from teacher where staff_typ='teaching'");
...
{
$myArr=array($i,$res['teacher_name'],$res['father_name'],$res['contact'],$res['teacher_qualifi'],$res['address']);
$excel->writeLine($myArr);
$i++;
}
}
...
<table class="table table-bordered" id="sample_1" style="font-size:12px; ">
			  ...
			  <tbody>
			  <?php
    $sql=mysqli_query($con,"select * from teacher where status='Inactive' and teacher_session='".$_SESSION['session']."'");
	...
	while($row=mysqli_fetch_array( $sql))
	{
		?>
               <tr>
                  <td><?php echo $i;  ?></td>
				  <td><img src="uploads/<?php echo $row["timg"]; ?>" style="border-radius:5px; width:50px; height:60px;"></td>
			    ...
			    <a href="javascript:void(0)" style="color:#FF0000" onClick="return popitup('https://smarterponline.com//shining/school/cer.php?tid=<?php echo $row['id']."&ses=".$_SESSION['session'];  ?>')">Ex. Certificate</a>
			  </td>
                  </tr>
