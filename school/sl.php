...
.row-fluid .span6 {
 ...
 {
 $id = $_GET['stdid'];
 $delete=mysqli_query($con,"delete from fee_db where id='".$_GET['did']."'");
 }
 ?>
 ...
 if(isset($_POST['search4']))
				{
				 ...
				 $studrow=mysqli_fetch_array($search);
				 $memo=mysqli_query($con,"select * from fee_db where session='".$_SESSION['session']."' and student='".$studrow['student_id']."' ");              
				 $num=mysqli_num_rows($memo);
			  ...
			  // }
			   ...
			   if(!empty($_GET['stdid']))
			   {
				 ...
				 $studrow=mysqli_fetch_array($search);
				 $memo=mysqli_query($con,"select * from fee_detail where session='".$_SESSION['session']."' and school='".$_SESSION['uid']."' and student='".$studrow['student_id']."' ");                $num=mysqli_num_rows($memo);
		  ...
		  $memo1=mysqli_query($con,"select * from fee_detail_trans where session='".$_SESSION['session']."' and student='".$studrow1['student_id']."' ");
...
<form method="post" name="myForm" action="#" enctype="multipart/form-data"  onsubmit="return(validate());">
            ...
            <div class="box-head">
			<a style=" border-radius:5px; padding:5 5 5 5 ;color:#FFFFFF;font-size:16px" href="<?php echo $var."sl"; ?>">Search Student By Class</a>
			</div>
	   ...
	   <div id="dvContainer1">
	   <h2 style="font-weight:bold; color:#CC0000"><center>Student Fee Ledger</center> </h2>
    ...
    <td><?php echo $rowmemo['current_month'];  ?></td>
    <td><?php echo $rowmemo['reg'];?></td> 
    <td><?php echo $rowmemo['dbf'];?></td>
    <td><?php echo $rowmemo['fee_deposit'];?></td>
    <td><?php echo date("d-m-Y",strtotime($rowmemo['date']));?></td> 
    <td>
   
    <a href="<?php echo $var."sl&&did=".$rowmemo['id']; ?>" onClick="return confirmation();" style="color:#FF0000">
