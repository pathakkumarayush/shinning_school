...
.col_4{ width:100%; height:1700px; margin-left:2px; background-color:#FFFFFF;float:left; margin-top:10px;-webkit-box-shadow: 0 0 10px rgba(0,0,0, .65);
...
<div class="col_4" style="margin-top:0px; min-height:400px;" >
  ...
  <div style="width:95%; height:100px; border:1px solid black" id="msg" >
    ...
    <div style="border:#FF0000 0px solid; margin:270px -250px 0px 0px; width:250px; float:right">
  ...
  <textarea id="m_1" rows="10" data-status-message="#counter" cols="30" class="form-control" style="height:100px;"></textarea>
   <div id="counter" class="text-right"></div>
 	...
 	<table style="margin:30px 0px 0px 70px; font-size:14px; width:300px">
         ...
         <tr>
              ...
              <?php
                $class=mysqli_query($con,"select distinct(class) from class");
			 ?>
			...
			?>
			 ...
			 <?php /*?> <td><div id="txtHint1"></div></td><?php */?>
   ...
   <input type='checkbox' value='on' id='chkall' name='allbox' onclick='checkAll();'/>
    </td><td>&nbsp;Sr.No</td><td>&nbsp;NAME</td><td>&nbsp;Father</td><td>&nbsp;Class</td><td>&nbsp;Contact</td></tr>
	<?php
	$i=0;
	if(!empty($_REQUEST['section']))
	{
	$_REQUEST["section"]="";
	  $res_all=mysqli_query($con,"select * from student where student_class='".$_REQUEST["class"]."' and student_section='".$_REQUEST["section"]."' and student_school='".$_SESSION["uid"]."' and student_session='".$_SESSION['session']."' and status='0' order by student_name Asc")or die(mysqli_error());
	
	}
	else
	 {
	 $sec1="";
	$res_all=mysqli_query($con,"select * from student where student_class='".$_REQUEST["class"]."' and student_section='$sec1' and student_school='".$_SESSION["uid"]."' and student_session='".$_SESSION['session']."' and status='0' order by student_name Asc")or die(mysqli_error());
...
<?php
	...
	$sec="";
	   ...
	   $res=mysqli_query($con,"select * from class where session='".$_SESSION['session']."'")or die(mysqli_error());
      ...
      <?php
	   ...
	   ?>
         ...
         <tr>
                <td>Class<span class="textfieldRequiredMsg"></span></td>
              <?php
                $class=mysqli_query($con,"select distinct(class) from class where session='".$_SESSION['session']."'");
			 ?>
            <td><select name="class" class="select" style="width:125px" onChange="showSection(this.value)">
               <option value="-1">Select class</option>
              <?php
