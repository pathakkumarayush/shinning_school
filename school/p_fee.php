...
{ 
    ...
    <?php
      ...
      {
      echo 'ok';
      $query=mysqli_query($con,"insert into privious_fee(cid,sid,amt,session,rmk,ac_no)values('".$_POST['class']."','".$_POST['stdid']."','".$_POST['pfee']."','".$_SESSION['session']."','".$_POST['rmk']."','".$_POST['acount_no']."')");
	 ...
	 ?>
	         ...
	         <?php
              ...
              {
              $d=date("Y-m-d");
              $query=mysqli_query($con,"DELETE FROM privious_fee where id='".$_GET['did']."' and session='".$_SESSION['session']."'");	 
              }
			  ?>
<style>
...
<a href="./?pageid=pc_fee" style="color:#FFFFFF;float:right; background-color:#009966; margin-top:10px; padding:6px; font-size:18px">Class wise Report Fee</a>
</div>
<div class="col_4">
  ...
  <form method="post" name="myForm" action="#" enctype="multipart/form-data"  onsubmit="return(validate());">
		  ...
		  Add Previous Year Fee
			...
			?>
			 ...
			 <tr>
			    <td>Account No</td>
				<td><input type="text" name="acount_no" /></td>
			 </tr>
			 <tr>
			...
			<tr>
			 ...
			 <tr>
			    <td>Remarks</td>
				<td><input type="text" name="rmk" style="width:400px;"/></td>
			 </tr>
			 <tr>
			    <td>&nbsp;</td>
				<td>&nbsp;</td>
	  ...
	  <?php
	   ...
	   <div style="width:990px; margin-left:30px; height:400px; overflow:scroll">
		<a href="javascript:void(0)" style="color:#FF0000" onClick="return popitup('https://smarterponline.com/shining/school/feelist.php?ses=<?php echo $_SESSION['session']; ?>')"><input type="button" value="Print List " style="width:100px; position:absolute"></a>
		<table border="1" cellpadding="0" cellspacing="0" style="width:973px; font-size:14px;">
       ...
       <tr style="color:#fff; line-height:15px; color:#000000" align="center">
		...
		<td align="left">&nbsp;&nbsp;<?php echo $stdrow['student_fname'] ?? ''; ?></td>
       <td><?php echo $stdrow['student_class']; ?></td>
	    <td><?php echo $stdrow['student_contactno']; ?></td>
	   <td><?php echo $enquiryrow['rmk']; ?></td>
	   <td><?php echo ucwords($enquiryrow['amt']); $tdue+=(float)$enquiryrow['amt']; ?></td>
	   
	   <td>
<?php
$search1=mysqli_query($con,"select sum(fee_deposit),sum(concession),sum(latefee) from fee_detail_preivios where student='".$enquiryrow['sid']."' ");
...
?>			
	...
	<td><?php echo $balt = (float)$enquiryrow['amt'] - (float)$depo-(float)$tcon+(float)$tlate; $tbalt+=$balt; ?></td>
	
	
	
	
	    <td> 
	    <a href="<?php echo $var."p_fee&did=".$enquiryrow['id']; ?>" onClick="return confirmation();">Delete</a>
