...
<div class="full_div">
<br clear="all" />
<div class="left_sect"><img src="images/Pay Roll/staff.png" /><a href="./?pageid=home">
<img src="images/buttonGoBack.png"  style="float:right; width:150px; height:60px;"/></a></div>
<div class="shell">
<div class="shell_main">
<div class="enquiry">
<img src="images/tech.png"  style=" float:left; width:60px; height:40px;"/>
<h2 style="float:left; margin-left:10px; text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">Complaint Details</h2>

<a href="./?pageid=complaint" style="color:#FFFFFF;float:right; margin-left:0px; background-color: #CC0000; margin-top:10px; padding:6px; font-size:18px">Add New Complaint </a>
</div>
<?php
if(!empty($_GET['did']))
{
$delete=mysqli_query($con,"delete from student_complaint where id='".$_GET['did']."' and session='".$_SESSION['session']."'");
?>
                <script>
		        alert('Complaint Delete Successfully');
                window.location.href='https://smarterponline.com/shining/school/?pageid=view_complaint';
                </script>
...
?>	
<?php
$maxid=mysqli_query($con,"select count(id) from student_complaint where session='".$_SESSION['session']."'");
$maxrow=mysqli_fetch_array($maxid);
$rowmax=mysqli_fetch_array($maxid);
?>
<div class="col_4">
<div class="form-style-2-heading">
Total Complaint : <?php echo $maxrow['count(id)']; ?>
</div>
<table class="table table-bordered" id="sample_1" style="font-size:12px; ">
			  ...
			  <tbody>
			  <?php
              $sql=mysqli_query($con,"select * from student_complaint where session='".$_SESSION['session']."'");
	          $i=1;
			  ...
			  $uid=$row['teacher'];
			  $sqlt=mysqli_query($con,"select * from teacher where uid='$uid' and teacher_session='".$_SESSION['session']."'");
			  $rowt=mysqli_fetch_array($sqlt);
		      ?>
               <tr style="line-height:20px;">
                  <td><?php echo $i;  ?></td>
                  <td><?php echo $row['complaint_by'] ?></td>
                  <td><?php echo $rowt['teacher_name'] ?></td>
	              ...
	              <td><a href="<?php echo $var."view_complaint&did=".$row['id']; ?>" onClick="return confirmation();">Delete</a> </td>
