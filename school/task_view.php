...
<h2 style="float:left; margin-left:10px; text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">Teacher Task Details</h2>

<a href="./?pageid=task" style="color:#FFFFFF;float:right; margin-left:0px; background-color: #CC0000; margin-top:10px; padding:6px; font-size:18px">Assign New task </a>
</div>
<?php
if(!empty($_GET['did']))
{
$delete=mysqli_query($con,"delete from tasks where id='".$_GET['did']."' and session='".$_SESSION['session']."'");
} 
?>
<?php
$maxid=mysqli_query($con,"select count(id) from tasks where session='".$_SESSION['session']."'");
$maxrow=mysqli_fetch_array($maxid);
$rowmax=mysqli_fetch_array($maxid);
?>
<div class="col_4">
<div class="form-style-2-heading">
Total Task : <?php echo $maxrow['count(id)']; ?>
</div>
<table class="table table-bordered" id="sample_1" style="font-size:12px; ">
			  ...
			  <tbody>
			  <?php
              $sql=mysqli_query($con,"select * from tasks where session='".$_SESSION['session']."'");
	          $i=1;
		      ...
		      ?>
               <tr style="line-height:20px;">
	              ...
	              <td><a href="<?php echo $var."task_view&did=".$row['id']; ?>">Delete</a> </td>
