...
<div class="full_div">
        ...
        <h2 style="float:left; margin-left:10px; text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">Scholar Register Update</h2>
		...
		<!-- <div style="width:1322px; height:50px; background-color:#FF0000; margin-top:10px;">
		 ...
		 <table style="width:100%;color:#000000">
		    ...
		    <tr style="color:#FFFFFF;text-align: center; background-color:#006699;">
             <th>STUDENT NAME</th>
			 <th>CLASS</th>
			 <th>Date of Admission<br> in the class</th>
             <th>Date of Passing Class<br>or Leaving the school</th>
             <th>School Day</th>
             <th>Present Day</th>
			 <th>No. of Student<br> in the class</th>
			 <th>Scholar Serial <br>in Annual Exam</th>
			 <th>Particular<br> Character</th>
	         <th colspan="2">Action</th>
            </tr>
            </thead>
		 <?php
		 ...
		 {
		 $searchs=mysqli_query($con,"select * from sch_reg where sid='".$studrow['student_id']."' and class='".$studrow['student_class']."' and ses='".$_SESSION['session']."'");
		 ...
		 <td >
          <input type="text" name="doa" value="<?php echo $studrows['doa']; ?>" id="doa<?php echo $i;?>" style="width:70px;">
         </td>
		 <td >
            <input type="text" name="dop" value="<?php echo $studrows['dop']; ?>" id="dop<?php echo $i;?>" style="width:70px;">
         </td>
		 <td >
            <input type="text" name="w_day" value="<?php echo $studrows['w_day']; ?>" id="w_day<?php echo $i;?>" style="width:70px;">
         </td>
		 <td >
            <input type="text" name="p_day" value="<?php echo $studrows['p_day']; ?>" id="p_day<?php echo $i;?>" style="width:75px;">
         </td>
		 
		  <td >
            <input type="text" name="ts_class" value="<?php echo $studrows['ts_class']; ?>" id="ts_class<?php echo $i;?>" style="width:70px;">
         </td>
		 
		 
		 
		  <td >
            <input type="text" name="ss_exam" value="<?php echo $studrows['ss_exam']; ?>" id="ss_exam<?php echo $i;?>" style="width:70px;">
         </td>
		
	     <td >
            <input type="text" name="pcha" value="<?php echo $studrows['pcha']; ?>" id="pcha<?php echo $i;?>" style="width:100px;">
         </td>
		 
		 <td >
        <?php if(!empty($studrows['doa']) || !empty($studrows['dop']) || !empty($studrows['w_day']) || !empty($studrows['p_day']) || !empty($studrows['ts_class']) || !empty($studrows['ss_exam']) || !empty($studrows['pcha']) ){ ?>
...
var doa= document.getElementById('doa'+hid).value;
var dop= document.getElementById('dop'+hid).value;
var w_day= document.getElementById('w_day'+hid).value;
var p_day= document.getElementById('p_day'+hid).value;
var ts_class= document.getElementById('ts_class'+hid).value;
var ss_exam= document.getElementById('ss_exam'+hid).value;
var pcha= document.getElementById('pcha'+hid).value;



var data_str= "cls="+cls+"&idm="+idm+"&doa="+doa+"&dop="+dop+"&w_day="+w_day+"&p_day="+p_day+"&ts_class="+ts_class+"&ss_exam="+ss_exam+"&pcha="+pcha;
