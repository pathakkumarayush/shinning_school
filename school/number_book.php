...
<div class="full_div">
        ...
        <h2 style="float:left; margin-left:10px; text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">NUMBER WORK</h2>
		...
		<!-- <div style="width:1322px; height:50px; background-color:#FF0000; margin-top:10px;">
		 ...
		 <table style="width:100%;color:#000000">
		    ...
		    <tr style="color:#FFFFFF;text-align: center; background-color:#006699;">
             <th style="padding: 10px;">STUDENT NAME</th><th style="width:104px;">CLASS</th><th style="width:95px;">Exam</th>
              <th>RECOGNITION</th>
              <th>CLARITY OF CONCEPTS</th>
			  <th>ABILITY TO ADD</th>
			  <th>ABILITY TO COUNT</th>
			   <th>FORMATION</th>
			  <th>SHAPES</th>
		 ...
		 {
		  $searchs=mysqli_query($con,"select * from num_book where student='".$studrow['student_id']."' and class='".$studrow['student_class']."' and exam='".$_POST['exam']."' and session='".$_SESSION['session']."'");
		 ...
		 <td >
			...
			<input type="hidden" name="subject" value="<?php echo $_POST['subject']; ?>" id="subject<?php echo $i;?>" style="width:60px;">
         </td>
		 ...
		 <td >
      <?php if(!empty($studrows['confidant']) || !empty($studrows['polite']) || !empty($studrows['reponsible']) || !empty($studrows['decipline']) || !empty($studrows['regular']) || !empty($studrows['appe']) ){ ?>
...
var data_str= "cls="+cls+"&idm="+idm+"&exam="+exam+"&confidant="+confidant+"&polite="+polite+"&reponsible="+reponsible+"&decipline="+decipline+"&regular="+regular+"&appe="+appe;
