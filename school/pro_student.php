...
<body alink="#00FF66" link="#00CC00">
              ...
              <?php
              $maxid=mysqli_query($con,"select count(student_id) from student where student_school='".$_SESSION["uid"]."' and student_session='".$_SESSION['session']."'and status='2'");
  
              $maxrow=mysqli_fetch_array($maxid);
  
              $rowmax=mysqli_fetch_array($maxid);

              $maxid2=mysqli_query($con,"select count(student_id) from student where student_school='".$_SESSION["uid"]."' and student_session='".$_SESSION['session']."'and status='2'");
              $maxrow2=mysqli_fetch_array($maxid2);

              if(isset($_POST['search1']))
			  {
					  
			  $search=mysqli_query($con,"select * from student where student_scholar='".$_POST['scholarno1']."' and student_school='".$_SESSION['uid']."' and status='2'");
			  ...
			  {
			  $search=mysqli_query($con,"select * from student where student_name Like '".$_POST['studentname']."%' and student_session='".$_SESSION['session']."' and status='2'");
		      $num=mysqli_num_rows($search);	   
			  }
			  ...
			  {
			  $search=mysqli_query($con,"select * from student where student_class='".$_POST['class']."' and  student_session='".$_SESSION['session']."' and status='2' order by student_name Asc");
			  ...
			  student_session='".$_SESSION['session']."' and status='2'");
			  $num=mysqli_num_rows($search);	   
	          } 
			  }
			  ?>
...
<div class="full_div">
        ...
        <center><h2 style="text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">Student Recycle</h2></center>
        </div>
        <div class="col_4">
	    ...
	    <a style=" border-radius:5px; padding:5 5 5 5 ;color:#FFFFFF;font-size:16px" href="<?php echo $var."pro_student"."&&divid=4"; ?>">Search Student By Class</a>
		
		 &nbsp;||&nbsp;
		 <a style=" border-radius:5px; padding:5 5 5 5 ;color:#FFFFFF;font-size:16px" href="<?php echo $var."pro_student"."&&divid=2"; ?>">Search Student By Name</a>
		</div>				
	 ...
	 <a  href="javascript:void(0)" style="color:#FF0000" onClick="return popitup('https://smarterponline.com/kmps/school/studentlist.php?student_class=<?php echo $_POST['class'];  ?>')">     <input type="button" value="Print List " style="width:100px; position:absolute"></a>
	    ...
	    <td><?php echo $studrow['student_address'];?></td>
		 <td><?php echo $studrow['rea'];?></td>
	    <td>
		<a href="<?php echo $var."pro_student&tid=".$studrow['student_id']; ?>" onClick="return confirmation();">Return</a>
		&nbsp;||&nbsp;
		<a href="<?php echo $var."admina&upstudid=".$studrow['student_id']; ?>"  target="_blank"><b>Reason</b> </td> 
        </tr>
    <?php
     $i++;
	 }
