...
<h2 style="float:left; margin-left:10px; text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">
 <a href="./?pageid=inventry_home" style="text-decoration:none">Inventory</a> ->Show Release Details</a></h2>
</div>
<div class="col_4" style="margin-top:0px;" >
            ...
            <div class="box-head">
						<h2 class="left">Currently Release Store Detail</h2>
				   </div>
           <div class="table" style="border:#FF0000 0px solid;">
		 ...
		 <th>Allocated To</th> 
			  ...
			  <tbody>
         <?php
         $memo=mysqli_query($con,"select * from receiving order by id desc");
		 $i=1;
         ...
         <td><?php echo $i;  ?></td>
         <td><?php 
	$item=mysqli_query($con,"select * from additem where id='".$rowmemo['item']."'");
    $r_item=mysqli_fetch_array($item);
    echo ucwords($r_item['item']); ?></td>
    <td><?php echo $rowmemo['quantity'];  ?></td>
	<td><?php echo $rowmemo['department'];  ?></td>   
    <td> <?php echo date("d-m-Y",strtotime($rowmemo['allocatedate']));  ?></td>
    <td><?php echo ucwords($rowmemo['allocateto']);  ?> </td>
    </tr>
    <?php
    $i++;
	}
