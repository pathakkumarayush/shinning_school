...
if(!empty($_GET['id']))
{
$getdetail=mysqli_query($con,"select * from fee_detail_board where student='".$_GET['id']."' and session='".$_SESSION['session']."' order by id desc limit 1");
...
<body>
	...
	<table class="table" style="margin:5px 0px 0px 1px; width:325px; font-size:12px;font-weight:bold;" border="0">
		  ...
		  <?php } ?>
		   ...
		   <?php
		   if(!empty($rowfeedetail['board_fee']))
		   {
		   ?>
		   <tr style="line-height:20px;">
		   <td>&nbsp;&nbsp;Board Fee</td>
		   <td>&nbsp;&nbsp; <?php echo $rowfeedetail['board_fee']; ?></td>
		   </tr>
		   <?php } ?>
		   
		   
		    <?php
		   if(!empty($rowfeedetail['practical_fee']))
		   {
		   ?>
		   <tr style="line-height:20px;">
		   <td>&nbsp;&nbsp;Practical Fee</td>
		   <td>&nbsp;&nbsp; <?php echo $rowfeedetail['practical_fee']; ?></td>
		   </tr>
		   <?php } ?>
