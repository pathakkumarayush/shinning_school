...
<h2 style="float:left; margin-left:10px; text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">
 <a href="./?pageid=inventry_home" style="text-decoration:none">Inventory</a> ->Available Stock</a></h2>
</div>
<div class="col_4" style="margin-top:0px; min-height:335px;" >
  ...
  <br><br>
            <div class="box-head">
						<h2 class="left">Available Stock Details</h2>
				   </div>
	...
	<table class="table table-bordered" id="sample_1" style="font-size:12px; ">
              <thead style="background-color:#009933; color:#FFFFFF">
               <tr style="background-color:#009933;color:#FFFFFF">
				  ...
				  <th>CATEGORIES</th>
                  <th>ITEM NAME</th>
                  <th>AVILAVALE QUANTITY</th>
				  </tr>
			  </thead>
			  <tbody>
			 <?php
      $memo=mysqli_query($con,"select * from additem");
	$i=1;
	while($rowmemo=mysqli_fetch_array($memo))
	{
	$ids = $rowmemo['store'];
	$cat=mysqli_query($con,"select * from  addstore where id='$ids'"); 
	$rowst=mysqli_fetch_array($cat);
	?>	
               <tr style="color:#335599">
    <td><?php echo $i;  ?></td>
    <td><?php echo $rowst['store']; ?></td>
