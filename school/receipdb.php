...
if(!empty($_GET['id']))
{
$getdetail=mysqli_query($con,"select * from fee_db where student='".$_GET['id']."' and session='".$_SESSION['session']."' order by id desc limit 1");
...
<body>
    ...
    <td><?php echo date("d-m-Y",strtotime($rowfeedetail['date']));  ?></td>
		  ...
		  <?php
		  if(!empty($rowfeedetail['dbf']))
		  {
		  ?>
		  <tr style="line-height:21px;">
		  <td>&nbsp;&nbsp;Dayboarding Fee</td>
		  <td align="right">&nbsp;&nbsp; <?php echo $rowfeedetail['dbf']; ?>&nbsp;&nbsp;</td>
		  </tr>
		  <?php } ?> 
