...
<div class="col_4" style="margin-top:0px; " >	
  ...
  <div style="width:95%; height:100px; border:1px solid black" id="msg" >
   ...
   <tr valign="top"><td style="border:none;">
	...
	$qry=mysqli_query($con,"select * from student where student_class='".$row_all['class']."' and student_section='".$row_all['class_section']."' and student_school='".$_SESSION['uid']."' and student_session='".$_SESSION['session']."' and status='0' and st_type='hpri' order by student_name Asc");
	   ...
	   {
	    $sec1="";
        $qry=mysqli_query($con,"select * from student where student_class='".$row_all['class']."' and  student_section='$sec1' and  student_school='".$_SESSION['uid']."' and student_session='".$_SESSION['session']."' and status='0' and st_type='hpri' order by student_name Asc");
...
<?php
   ...
   <input type='checkbox' value='on' id='chkall' name='allbox' onclick='checkAll();'/>
    </td><td>&nbsp;class</td><td>&nbsp;Total Student</td></tr>
	<?php
	$i=0;
	...
	while($row_all=mysqli_fetch_array($res_all))
	{
		...
		?>
        <tr <?php if($i%2==0){echo "bgcolor='#E0FADC'";} ?>>
        <td align="center"><input type="checkbox" name='formDoor[]' value="<?php echo $row_all["class_id"]; ?>"  id='chk<?php echo $i; ?>' /></td>
         <td>&nbsp;<?php echo $row_all["class"].$row_all["class_section"]; ?></td>
        <td>
		     <?php
	             $countclass=mysqli_query($con,"select count(student_id) from student where student_class='".$row_all["class"]."' and student_section='".$row_all["class_section"]."' and student_school='".$_SESSION['uid']."'");
				
			   $rowcount=mysqli_fetch_array($countclass);
			   echo $rowcount['count(student_id)']; 		 
			 ?>
	</td>
        </tr>
        <?php
