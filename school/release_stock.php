...
<?php
   ...
   {
   if(empty($_POST['quantity']) || empty($_POST['allocateto'])  || empty($_POST['date']))
   {
    $err="Field Marked With * are Mandatory";
   }
   if(empty($err))
   {
   	$item=mysqli_query($con,"select * from additem where id='".$_POST['item']."'");
    
	$r_item=mysqli_fetch_array($item);
   echo $r_item['quantity'];
    if(($r_item['quantity']>0) &&  ($_POST['quantity']<=$r_item['quantity']))
	{
   
    $query=mysqli_query($con,"insert into receiving(category,item,quantity,department,allocateto,allocatedate) values('".$_POST['store']."','".$_POST['item']."','".$_POST['quantity']."','".$_POST['department']."','".$_POST['allocateto']."','".$_POST['date']."')");
   $val=$r_item['quantity']-$_POST['quantity'];
   $update=mysqli_query($con,"update additem set quantity=$val where id='".$_POST['item']."'");
...
<h2 style="float:left; margin-left:10px; text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">
 <a href="./?pageid=inventry_home" style="text-decoration:none">Inventory</a> -> Release Stock</a></h2>
</div>
<div class="col_4" style="margin-top:0px; height:680px; " >
	 ...
	 if(!empty($msg))
		...
		<?php  } ?>
	           <br>
            <div class="box-head">
						<h2 class="left">Release Stock</h2>
				   </div>
       ...
       <td>
			...
			?>
              <select name="store" class="select" onchange="get_item(this.value)" style="width:220px;">
	...
	<td>&nbsp;</td>
		...
		<tr>
		<td>Select Item<span style="color:#FF0000">*</span></td>
	...
	<tr>
		<td>Quantity<span style="color:#FF0000">*</span></td>
		<td><input type="text" name="quantity" class="tb5"></td>  
		</tr>
		<tr>
    <td>&nbsp;</td>
	<td>&nbsp;</td>
	...
	<tr>
		...
		<td>
		       <?php
				  ...
				  ?>
				  <select name="department" style="width:220px;" class="select">
				  <option>Select Department</option>
				  <?php
			     while($rowdepartment=mysqli_fetch_array($department))
				 {
				 ?>
				 <option value="<?php echo $rowdepartment['name'];  ?>"><?php echo $rowdepartment['name'];  ?></option>
				 <?php
				 }
			   ?>
		         </select>
	...
	<td>&nbsp;</td>
			...
			<tr>
		<td>Allocatted To<span style="color:#FF0000">*</span></td>
		<td><input type="text" name="allocateto" class="tb5"></td>  
		</tr>
		<tr>
    <td>&nbsp;</td>
	<td>&nbsp;</td>
			...
			<tr>
		<td>Allocatted Date<span style="color:#FF0000">*</span></td>
		<td><input type="text" name="date" class="tb5"></td>  
		</tr>
		<tr>
    <td>&nbsp;</td>
	<td>&nbsp;</td>
