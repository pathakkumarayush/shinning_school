...
if(!empty($_SESSION['sumsg']))
{
  unset($_SESSION['sumsg']);
				...
				/* $ckfee=mysqli_query($con,"select * from fee_detail_hostel where student='".$studrow['student_id']."' and session='".$_SESSION['session']."' and month='".$_SESSION['month1']."'");
	            $numchk=mysqli_num_rows($ckfee);
				*/
				
				$distinctmonth=mysqli_query($con,"select distinct(month) from fee_detail_hostel where student='".$studrow['student_id']."' and school='".$_SESSION['uid']."' and session='".$_SESSION['session']."'");
...
$search=mysqli_query($con,"select * from fee_detail_hostel where session='".$_SESSION['session']."' and school='".$_SESSION['uid']."' and student='".$studrow['student_id']."'");
$num=mysqli_num_rows($search);
		...
		//query to check previous month fee
		 ...
		 /*
			  $prevfee=mysqli_query($con,"select * from fee_detail_hostel where month='".$rowprev2['month']."' and student='".$studrow['student_id']."' and school='".$_SESSION['uid']."' and session='".$_SESSION['session']."'");
...
if((!empty($_POST['latefee1']))|| (!empty($_POST['concession'])) )
{
	  ...
	  if($_POST['montha']=='April,November')
	  {
	  ...
	  $m=date("M");
$receptno=mysqli_query($con,"select * from fee_detail_hostel where receiptno='".$_POST['srecipt']."' and session='".$_SESSION['session']."'");
if(mysqli_num_rows($receptno)<1)
{

$query=mysqli_query($con,"insert into fee_detail_hostel(session,class,adm_fee,caution,inst_fee,inst_fee_bus,tamnt,tpay,due,pdue,pay_type,fee_deposit,month,student,date,latefee,concession,school,receiptno,current_month,
instalment,extra_amnt,remark,cno,cd,padv,anual)
...
'".$_POST['cd']."','".$_POST['padv']."','".$_POST['anual']."')");
		...
		$update=mysqli_query($con,"update fee_detail_hostel set admissionfee='Yes' where id='$insertid' and school='".$_SESSION['uid']."'");
		} 
		...
		$nmsg="Thanks for Paying Hostel Fee For the $instn. Received amount ".$_POST['amntdeposit'];	
...
#div2{ display:none;}
 ...
 <div class="full_div">
        ...
        <h2 style="float:left; margin-left:10px; text-transform:uppercase; color:#006633; font-size:20px; margin-top:15px;">Student Hostel Fee Pay</h2>
...
<div class="col_4">
	...
	<form method="post" name="myForm" action="#" enctype="multipart/form-data"  onsubmit="return validateForm()" >
		  ...
		  <a style=" border-radius:5px; padding:5 5 5 5 ;color:#FFFFFF;font-size:16px" href="<?php echo $var."pay_fee_hostel"."&&divid=3"; ?>">Search Student By Class</a>
          ...
          $search=mysqli_query($con,"select * from student where student_school='".$_SESSION['uid']."' and status='0' and rti='No' and student_class='".$rclass['class']."' 
		  ...
		  <input type="checkbox" name="month1[]" value="November"><span style="color:#fff; font-weight:bold">Instalment4</span>-November <br>
          </div>
		  </td>
		 ...
		 <div id="state-list">
		  ...
		  <td>
		  <?php
		  $maxid=mysqli_query($con,"select max(id) from fee_detail_hostel where school='".$_SESSION['uid']."'");
		  ...
		  <?php
		  $i=1;
		  $selrc=mysqli_query($con,"select * from instdetail_hostel where class='".$studrow['student_class']."' and session='".$_SESSION['session']."' and month='$m'");
		  ...
		  <tr>
		  <td><?php echo $i; ?></td>
		  <td>Hostel Tution Fee (<?php echo $rowselrec['inst_type']; ?>)</td>
		  ...
		  <td><b><?php   $search1=mysqli_query($con,"select * from fee_detail_hostel where session='".$_SESSION['session']."' and school='".$_SESSION['uid']."' 
...
?>
<td>
<a href="javascript:void(0)" style="color:#FF0000" onClick="return popitup('https://smarterponline.com/kmps/school/getreceipt_h.php?id=<?php echo $_SESSION['studentid']; ?>')">
